function T = stack_of_spirals(R,Nradial,Nz,fov,resolution, pf, slew_max, g_max)
...
if ~exist('slew_max','var')
    slew_max = 170;
end

SYSTEM=GradSystemStructure('custom', [], slew_max, g_max);
    ...
    for iz=1:Nz
        ...
        for element=2:size(T, 1)
            if element == (size(T, 1)-3)/2 || element == (size(T, 1)-1)/2 || element == (size(T, 1)+1)/2 || element == (size(T, 1)+3)/2
                T(element,iz,iradial).SYS = GradSystemStructure('custom', [], 150, g_max);
                Tc(iz,iradial).SYS = T(element,iz,iradial).SYS;
            else
                T(element,iz,iradial).SYS = GradSystemStructure('custom', [], slew_max, g_max);
                Tc(iz,iradial).SYS = T(element,iz,iradial).SYS;
            end
           Tc(iz,iradial) = trajectStruct_connect(Tc(iz,iradial),T(element,iz,iradial), 'rip'); % test AB
